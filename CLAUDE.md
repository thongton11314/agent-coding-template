...
— delegate the entire task to the `agent-developer` subagent
...
"show me W" — delegate to the `agent-explorer` subagent (`.claude/agents/agent-explorer.md`).

Only answer directly (without delegating) for pure conceptual questions that
require no file access.

> Claude Code auto-discovers subagents in `.claude/agents/`. Invoke a subagent by
> name (for example, "use the agent-developer subagent to ...") or via the Task
> tool. The subagent files mirror their counterparts in `.github/agents/` so the
> behavior is identical across VS Code Copilot and Claude Code.
