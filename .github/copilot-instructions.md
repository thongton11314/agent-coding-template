...
changing any file in `src/`, `tests/`, `ui/`, `wiki/`, `config/`, or `scripts/`
...
- `SECURITY.md` — Ongoing security checklist (applied per-change, not just at Phase 5).
...
2. **Sync Gate** — Code-Wiki Mapping Table + bidirectional verification (Workflow 10)
...
10. **Sync Gate is mandatory** after every code change (Workflow 10).
...
| **Orchestration** | New app build | Copy `framework-template.md` → fill placeholders → place in `raw/` → run 8-phase pipeline |
| **Brownfield** | Existing codebase | Run Workflow 11 from AGENTS.md → back-fill wiki → start Phase 7 |
